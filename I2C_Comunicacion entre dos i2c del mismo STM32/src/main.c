/*********************************************************************
 ...
 EL MAESTRO LE DICE AL ESCLAVO QUE MUESTRE O MODIFIQUE UNA DATOS EN UNA POSICION
 DE MEMORIA. Y EL MAESTRO MANDA ESO POR EL UART.
...
#define I2CMASTER_ADDR 25 // LA DIRECCION, PUEDE SER ENTRE LA 1 Y 127, PUEDE SER LA QUE QUIERA
#define I2CSLAVE_ADDR  36 // LA DIRECCION
#define I2C1_CLOCK_FRQ 100000 // LA FRECUENCIA QUE UNO QUIERA

// estados de la comunicación con el esclavo, ES POR QUE SE HACE COMUNICACION POR MAQUINA DE ESTADOS.
...
void estado(I2C_TypeDef* i2c_cual); // MIRA LAS BANDERAS DEL ESTADO
void I2C1_Esclavo_init(void); // CONFIGURACION DEL I2C
...
void I2C1_ER_IRQHandler(void); // USADA EN CASO DE ERROR
void WriteByte(uint8_t address, uint8_t data); // ESCRIBIR UN BYTE EN MEMORIA
uint8_t mem_read(uint8_t reg_addr); // LEER UN BYTE
void RTCConfig(void); // RTC

uint8_t i2c1_mode;
uint8_t i2c1_ram_adr;
uint8_t memoria[LONGITUD_MEMORIA]; // MEMORIA DE ACCESSO UNICO DEL ESCLAVO.
...
int main(void)
{
    ...
    i2c1_mode = I2C1_MODE_WAITING; // INICIA EN EL ESTADO DE ESPERA
    i2c1_ram_adr= 0; // LA POSICION DE MEMORIA SERIA LA CERO


    // llena la memoria con -
    ...
    uint32_t reloj= RTC_GetCounter(); // toma el valor del rtc en ese momento
  ...
  while(1)
  {
      ...
      if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE)== SET)
      {
          ...
          // ? es para mostrar el contenido de la memoria, AQUI NO HAY COMUNICACION ENTRE I2C
          if(leer_uart== '?')
          {
              UART_mensaje("?\r\n", 3);
              for(i= 0; i< LONGITUD_MEMORIA; i++)
              {
                  // muestra la posicion de memoria
                  UART_mensaje("M(", 2);
                  UART_numero(i);
                  UART_mensaje(")= ", 3);
          ...
          // ¿ es para escribir en la memoria, AQUI SI HAY COMUNICACION ENTRE I2C
          if(leer_uart== '¿')
          {
              UART_mensaje("¿\r\n", 3);
              i= 0;
              ...
              {
                  if(USART_GetFlagStatus(USART1, USART_FLAG_RXNE)== SET) // bandera para saber si llego algo de teclado
                  {
                      caracter= (uint8_t) (USART_ReceiveData(USART1)); // leo lo llegado por teclado
...
void I2C1_Esclavo_init(void)
{
    ...
    I2C_InitStructure.I2C_OwnAddress1 = I2CSLAVE_ADDR; // <---- AQUI ES DONDE SE COLOCA Q ES ESCLAVO.
...
void I2C1_EV_IRQHandler(void)
{
    ...
    switch(event)
    {
        case I2C_EVENT_SLAVE_RECEIVER_ADDRESS_MATCHED: // el maestro ha enviado la dirección del esclavo para envío de datos al esclavo
        {
        ...
        case I2C_EVENT_SLAVE_BYTE_RECEIVED: // aqui entra cuando el maestro envia algo, y dependiendo si es direccion memoria o dato responde y cambia de estado. y guarda el dato en wert
        {
...
//                (void)(I2C1->SR2);
            ...
            // revisa dirección de memoria donde escribir
        ...
        case I2C_EVENT_SLAVE_TRANSMITTER_ADDRESS_MATCHED:// el maestro ha enviado la dirección del esclavo para leer un byte del esclavo
        {
        ...
        case I2C_EVENT_SLAVE_BYTE_TRANSMITTING: // aqui entra cuando el slave manda el dato al maestro.
        {
...
/***********************************************
 *  Esta función es para mostrar el estado de las banderas del i2c.
...
/***********************************************
 *  El maestro Manda un dato a la dirección address por i2c al esclavo
 ***********************************************/
void WriteByte(uint8_t address, uint8_t data)
{
  ...
  I2C_Send7bitAddress(I2C2, I2CSLAVE_ADDR, I2C_Direction_Transmitter); // voy a transmitirte info
